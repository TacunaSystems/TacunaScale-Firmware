...
// continuousReadAverageDelay() function
// Returns the average of several conversion results while waiting a specific time between readings
// --------------------
uint32_t PRDC_AD7193::continuousReadAverageDelay(uint32_t sampleNumber, uint32_t delay_ms) {
  #ifdef DEBUG_AD7193
    Serial.println(F("continuousReadAverageDelay()"));
  #endif
